...
case "$cmd" in
  ...
  create_share)
    ...
    if findmnt -no OPTIONS --target "$path" 2>/dev/null | grep -qw "ro"; then
        echo "ERROR: путь '$path' находится на файловой системе, смонтированной только для чтения (например, смонтированный ISO-образ) — панели нужно назначить владельца и права на папку шары, а на read-only ФС это невозможно. Вместо шары прямо на точке монтирования ISO: создай обычную (writable) шару на пути внутри /srv, а сам ISO смонтируй в подпапку ВНУТРИ неё (например, /srv/имя-шары/образ) — тогда содержимое ISO будет доступно через ту же шару, но сама шара останется на нормальной ФС." >&2
        exit 1
    fi
  ...
  disk_smart_summary)
    ...
    lsblk -P -o PATH,TYPE 2>/dev/null | grep 'TYPE="disk"' | while IFS= read -r line; do
        ...
        health="UNKNOWN"
        echo "$smart_out" | grep -qi "test result: *PASSED" && health="PASSED"
        echo "$smart_out" | grep -qi "test result: *FAILED" && health="FAILED"
        temp="$(echo "$smart_out" | awk '/Temperature_Celsius/ {print $10; exit} /^Temperature:/ {print $2; exit}' | grep -oP '^\d+' || true)"
...
EOF
  ...
  apply_update)
    ...
    # Гарантированный перезапуск сервиса панели — НЕ полагаемся только на
    # то, что сделал install.sh скачанного релиза (там уже должен быть
    # `systemctl restart`, а не `enable --now`, но старые релизы, выпущенные
    # до этого исправления, могли этого не делать — на "enable --now" уже
    # работающий сервис не перезапускается, файлы обновляются на диске, а в
    # памяти работающего gunicorn остаётся старый код до явного рестарта).
    # Этот процесс запущен отвязанным от текущего запроса панели (см.
    # api_apply_update в app.py — start_new_session=True), поэтому рестарт
...
EOF
  ...
  list_iso_files)
    ...
    find "$path" -maxdepth 3 -iname "*.iso" -type f 2>/dev/null | sort | while read -r f; do
        ...
        size="$(stat -c %s "$f" 2>/dev/null || echo 0)"
        mounted_loop="$(losetup -j "$f" 2>/dev/null || true)"
        if [[ -n "$mounted_loop" ]]; then
            loop_dev="${mounted_loop%%:*}"
            iso_mountpoint="$(lsblk -no MOUNTPOINT "$loop_dev" 2>/dev/null | head -1 || true)"
            mounted_flag=1
        else
            iso_mountpoint=""
            mounted_flag=0
        fi
        mountpoint_b64="$(printf '%s' "$iso_mountpoint" | base64 -w0)"
        echo "ISOFILE|${f_b64}|${size}|${mounted_flag}|${mountpoint_b64}"
    done || true
    ;;
